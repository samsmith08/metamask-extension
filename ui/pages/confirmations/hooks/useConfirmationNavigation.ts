...
export function navigateToConfirmation(
  ...
  if (hasApprovalFlows && hasNoConfirmations) {
    navigate(`${CONFIRMATION_V_NEXT_ROUTE}`);
    return;
  }
  ...
  if (CONNECT_APPROVAL_TYPES.includes(type)) {
    navigate(`${CONNECT_ROUTE}/${confirmationId}`);
    return;
  }
  ...
  if (type === ApprovalType.WatchAsset && !tokenId) {
    navigate(`${CONFIRM_ADD_SUGGESTED_TOKEN_ROUTE}`);
    return;
  }

  if (type === ApprovalType.WatchAsset && tokenId) {
    navigate(`${CONFIRM_ADD_SUGGESTED_NFT_ROUTE}`);
  }
}
