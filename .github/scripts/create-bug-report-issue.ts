...
import {
  ...
  GithubProject,
  GithubProjectField,
  retrieveGithubProject,
  updateGithubProjectDateFieldValue,
} from './shared/project';
...
async function main(): Promise<void> {
  ...
  const projectNumber = Number(
    process.env.RELEASES_GITHUB_PROJECT_BOARD_NUMBER,
  );
  if (!projectNumber) {
  ...
  const projectViewNumber = Number(
    process.env.RELEASES_GITHUB_PROJECT_BOARD_VIEW_NUMBER,
  );
  ...
  if (!isValidVersionFormat(releaseVersion)) {
    core.setFailed(
      `Invalid format for RELEASE_VERSION: ${releaseVersion}. Expected format: x.y.z`,
    );
    process.exit(1);
  }
  ...
  const regressionLabelTesting: Label = craftRegressionLabel(
    RegressionStage.Testing,
    releaseVersion,
  );
  const regressionLabelProduction: Label = craftRegressionLabel(
    RegressionStage.Production,
    releaseVersion,
  );
  const teamLabel: Label = craftTeamLabel(`${platformName}-platform`);

  // Create or retrieve the different labels
  await createOrRetrieveLabel(
    octokit,
    repoOwner,
    codeRepoName,
    regressionLabelProduction,
  );
  await createOrRetrieveLabel(
    octokit,
    repoOwner,
    codeRepoName,
    regressionLabelTesting,
  );
  await createOrRetrieveLabel(
    octokit,
    repoOwner,
  ...
  const regressionLabelTestingId = await createOrRetrieveLabel(
    octokit,
    repoOwner,
    planningRepoName,
    regressionLabelTesting,
  );
  const typeBugLabelId = await createOrRetrieveLabel(
    octokit,
    repoOwner,
    planningRepoName,
    typeBugLabel,
  );
  const teamLabelId = await createOrRetrieveLabel(
    octokit,
    repoOwner,
    planningRepoName,
    teamLabel,
  ...
  const issueWithSameTitle = await retrieveIssueByTitle(
    octokit,
    repoOwner,
    planningRepoName,
    issueTitle,
  ...
  if (issueWithSameTitle) {
    core.setFailed(
      `Bug report already exists: https://github.com/${repoOwner}/${planningRepoName}/issues/${issueWithSameTitle.number}. This is not desired, but can happen in cases where a release gets re-cut.`,
    );
    process.exit(1);
  }
  ...
  const issueId = await createIssue(
    ...
    issueTitle,
    issueBody,
    [regressionLabelTestingId, typeBugLabelId, teamLabelId],
  );

  // Retrieve project, in order to obtain its ID
  const project: GithubProject = await retrieveGithubProject(
    octokit,
    projectNumber,
  );
  ...
  const projectField: GithubProjectField | undefined = project.fields.find(
    (field) => field.name === projectFieldName,
  );

  if (!projectField) {
    throw new Error(
      `Project field with name ${projectFieldName} was not found on Github Project with ID ${project.id}.`,
    );
  }

  if (!projectField.id) {
    throw new Error(
      `Project field with name ${projectFieldName} was found on Github Project with ID ${project.id}, but it has no 'id' property.`,
    );
  }
  ...
  await updateGithubProjectDateFieldValue(
    octokit,
    project.id,
    projectField.id,
    issueId,
